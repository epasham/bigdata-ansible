...
do
    ...
    rsync -rv -e ssh --delete ~/filebeat/filebeat-$ES_VERSION-linux-x86_64-es-master root@$nodename:/root/filebeat
...
do
    echo "Filebeat BINARY SYNC es in $nodename -------------------------------------------------------------------------"
    rsync -rv -e ssh --delete ~/filebeat/filebeat-$ES_VERSION-linux-x86_64-es-client root@$nodename:/root/filebeat
    rsync -rv -e ssh --delete ~/filebeat/script root@$nodename:/root/filebeat
done
...
do
    echo "Filebeat BINARY SYNC es in $nodename -------------------------------------------------------------------------"
    rsync -rv -e ssh --delete ~/filebeat/filebeat-$ES_VERSION-linux-x86_64-es-master-and-data root@$nodename:/root/filebeat
    rsync -rv -e ssh --delete ~/filebeat/script root@$nodename:/root/filebeat
done
