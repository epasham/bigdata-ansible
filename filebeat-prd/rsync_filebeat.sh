...
do
    echo "Filebeat BINARY SYNC es in $nodename -------------------------------------------------------------------------"
    rsync -rv -e ssh --delete ~/filebeat/filebeat-$ES_VERSION-linux-x86_64-client root@$nodename:/root/filebeat
    rsync -rv -e ssh --delete ~/filebeat/script root@$nodename:/root/filebeat
done
...
do
    echo "Filebeat BINARY SYNC es in $nodename -------------------------------------------------------------------------"
    rsync -rv -e ssh --delete ~/filebeat/filebeat-$ES_VERSION-linux-x86_64-master-and-data root@$nodename:/root/filebeat
    rsync -rv -e ssh --delete ~/filebeat/script root@$nodename:/root/filebeat
done
